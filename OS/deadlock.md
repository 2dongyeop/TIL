...
## 💡 교착 상태로부터 회복 _Recovery from Deadlock
...
> 프로세스 또는 스레드를 중지시킴으로써 교착 상태를 제거하기 위해 우리는 두 방법 중 하나를 사용한다.
> 
- ***교착 상태 프로세스를 모두 중지***
    - 이 방식은 교착 상태의 사이클을 확실하게 깨뜨리지만, 그 비용이 크다.
    - 왜냐하면, 이들 프로세스가 오랫동안 연산했을 가능성이 있으며, 계산의 결과들을 폐기한다.
...
- ***교착 상태가 제거될 때까지 한 프로세스씩 중지***
    - 이 방법은 각 프로세스가 중지될 때마다 교착 상태 탐지 알고리즘을 호출해 프로세스들이 아직도 교착 상태인지를 확인하기 위해 상당한 오버헤드를 유발한다.

<br/>

<br/>

> 만일 부분 종료 방식을 사용한다면, 어느 프로세스를 중지해야 할 지 반드시 결정해야 한다.
> 
- 이러한 결정은 CPU 스케줄링 문제와 유사한 정책 결정의 문제이며, 기본적으로 경제적인 문제이다.
    - 즉, 프로세스들을
<br/>

<br/>
 중지시켰을 때 유발되는 비용이 최소인 프로세스들을 중지시켜야 한다!

> 어느 프로세스를 선택할지 결정하는 데는 아래와 같은 많은 요인이 있다.
> 
1. 프로세스의 우선순위가 무엇인지
...
만일 교착 상태를 해결학 위해 선점이 필요하다면, 아래의 세 가지 사항들을 고려해야 한다.
> 
...
1. **희망자 선택(selection of victim)**
    1. 어느 자원과 어느 프로세스들이 선점될 것인가를 결정해야 한다.
    2. 비용 요인 : 프로세스가 점유한 자원의 수, 프로세스가 지금까지 실행하는 데 소요한 시간 등등
    
    <br/>

2. **후퇴(rollback)**
    1. 프로세스로부터 자원을 선점하면, 해당 프로세스는 더 이상 자원이 없어 정상적으로 실행할 수 없어 진다.
    2. 따라서 이 경우, 프로세스를 안전한 상태로 후퇴시키고 그 상태로부터 다시 시작해야 한다.

    <br/>
    
1. **기아 상태(starvation)**
    1. 자원들이 동일한 프로세스로부터 항상 선점되지 않는다는 것을 어떻게 보장할까?
    2. 우리는 프로세스가 단지 (작은) 한정된 시간 동안만 희생자로 선정된다는 것을 반드시 보장해야 한다.
    3. 일반적인 해결 방법은 비용 요소에 후퇴의 횟수를 포함하는 방법이다.