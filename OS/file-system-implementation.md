...
# 💡 효율과 성능 _Efficiency and Performance
...
- 클러스터링
    - 이 기법은 내부 단편화를 허용하는 대신 파일 탐색과 파일 전송 성능을 향상시킨다.
    - 파일의 크기가 증가함에 따라 클러스터의 크기도 변화시킬 수 있다.

- 파일의 디렉토리 항목 내에 저장되는 자료의 유형을 고려해야 한다.
    - 최종 기록 날짜 등을 기록할때 → 판독될 때마다 기록해야 하는 부담이 있다!
...
- 버퍼 캐시 : 메인 메모리의 별도의 구역으로 유지
- 페이지 캐시 : 가상 메모리 기법을 사용해 파일 자료를 파일 시스템 지향의 블록이 아닌 페이지로 캐시
...
<img src="https://github.com/2dongyeop/TIL/blob/main/OS/image/14-12.png" width = 500/>
...
<img src="https://github.com/2dongyeop/TIL/blob/main/OS/image/14-13.png" width = 500/>
